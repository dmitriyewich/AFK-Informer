...
if limgui then
	...
	newFrame = imgui.OnFrame(
		...
		function(player)
			...
			if imgui.Checkbox('Вкл\\Выкл скрипт', active) then
			...
			imgui.SetCursorPosX((imgui.GetWindowWidth() - imgui.CalcTextSize('Текущий таймер: '..config.settings.time.. ' секунд').x) / 2)
			imgui.Text("Текущий таймер: "..config.settings.time.. ' секунд')
			-- imgui.SetCursorPosX((imgui.GetWindowWidth() - 215) / 2)
			imgui.PushItemWidth(217)
			if imgui.InputTextWithHint('##ID 2', 'Введите время в секундах', Timewaitsec, sizeof(Timewaitsec), imgui.InputTextFlags.CharsDecimal + imgui.InputTextFlags.AutoSelectAll + imgui.InputTextFlags.EnterReturnsTrue) then
				config.settings.time = str(Timewaitsec)
				savejson(convertTableToJsonString(config), "moonloader/config/AFK Informer.json")
			end
			imgui.PopItemWidth()
			imgui.SetCursorPosX((imgui.GetWindowWidth() - 130) / 2)
			if imgui.Button("Сохранить таймер", imgui.ImVec2(130, 0)) then
				config.settings.time = str(Timewaitsec)
				savejson(convertTableToJsonString(config), "moonloader/config/AFK Informer.json")
			end
			if imgui.Checkbox('Разворачивать окно игры', showwindow) then
				config.settings.showwindow = showwindow[0]
				savejson(convertTableToJsonString(config), "moonloader/config/AFK Informer.json")
			end
			if imgui.Checkbox('Показывать окно уведомления', pushwindow) then
				config.settings.pushwindow = pushwindow[0]
				savejson(convertTableToJsonString(config), "moonloader/config/AFK Informer.json")
			end
			if imgui.Checkbox('Звукое уведомления', soundwindow) then
				config.settings.soundwindow = soundwindow[0]
				savejson(convertTableToJsonString(config), "moonloader/config/AFK Informer.json")
			end
			if imgui.CollapsingHeader('Доп.Настройки') then
				imgui.SetCursorPosX((imgui.GetWindowWidth() - 210) / 2)
				imgui.PushItemWidth(210)
				if imgui.InputTextWithHint('##cmd', 'Введите команду без "/"', cmdbuffer, sizeof(cmdbuffer), imgui.InputTextFlags.AutoSelectAll) then
