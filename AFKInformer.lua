...
assert(lvkeys, 'Library \'vkeys\' not found.') -- Áèáëèîòåêà ñ êîäàìè êëàâèø â óäîáíîì ôîðìàòå
...
if limgui then
	...
	infoOK = ''
	if showwindow[0] and pushwindow[0] then infoOK = 'Íàæìèòå Äà, ÷òîáû ðàçâåðíóòü èãðó' else infoOK = '' end
	changecmdtext = u8''
		...
		function(player)
			...
			imgui.Begin("AFK Informer by dmitriyewich", afk_window, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize)
					...
					if os.clock() >= go_CMDserver then
						imgui.PushStyleVarFloat(imgui.StyleVar.Alpha, (alpha <= 1.0 and alpha or 1.0))
							...
							imgui.PushTextWrapPos(450)
								imgui.TextUnformatted(u8'©dmitriyewich aka Âàëåðèé Äìèòðèåâè÷.\nÐàñïðîñòðàíåíèå äîïóñêàåòñÿ òîëüêî ñ óêàçàíèåì àâòîðà\nÏÊÌ - Îòêðûòü ãðóïïó â âê')
				...
				if imgui.IsItemClicked(1) then
					os.execute('explorer "https://vk.com/dmitriyewichmods"') -- îòêðûòèå áðàóçåðà ñ ýòîé ññûëêîé
				end
			imgui.SetCursorPosY(imgui.GetCursorPosY() - 3)
			if imgui.Checkbox(u8'Âêë\\Âûêë ñêðèïò', active) then
				config.settings.active = active[0]
				savejson(convertTableToJsonString(config), "moonloader/config/AFK Informer.json")
			end
			if showwindow[0] and pushwindow[0] then infoOK = 'Íàæìèòå Äà, ÷òîáû ðàçâåðíóòü èãðó' else infoOK = '' end
			imgui.SetCursorPosX((imgui.GetWindowWidth() - imgui.CalcTextSize(u8'Òåêóùèé òàéìåð: '..config.settings.time.. u8' ñåêóíä').x) / 2)
			imgui.Text(u8"Òåêóùèé òàéìåð: "..config.settings.time.. u8' ñåêóíä')
			-- imgui.SetCursorPosX((imgui.GetWindowWidth() - 215) / 2)
			imgui.PushItemWidth(217)
			if imgui.InputTextWithHint(u8'##ID 2', u8'Ââåäèòå âðåìÿ â ñåêóíäàõ', Timewaitsec, sizeof(Timewaitsec), imgui.InputTextFlags.CharsDecimal + imgui.InputTextFlags.AutoSelectAll + imgui.InputTextFlags.EnterReturnsTrue) then
				config.settings.time = str(Timewaitsec)
				savejson(convertTableToJsonString(config), "moonloader/config/AFK Informer.json")
			end
			imgui.PopItemWidth()
			imgui.SetCursorPosX((imgui.GetWindowWidth() - 130) / 2)
			if imgui.Button(u8"Ñîõðàíèòü òàéìåð", imgui.ImVec2(130, 0)) then
				config.settings.time = str(Timewaitsec)
				savejson(convertTableToJsonString(config), "moonloader/config/AFK Informer.json")
			end
			if imgui.Checkbox(u8'Ðàçâîðà÷èâàòü îêíî èãðû', showwindow) then
				config.settings.showwindow = showwindow[0]
				savejson(convertTableToJsonString(config), "moonloader/config/AFK Informer.json")
			end
			if imgui.Checkbox(u8'Ïîêàçûâàòü îêíî óâåäîìëåíèÿ', pushwindow) then
				config.settings.pushwindow = pushwindow[0]
				savejson(convertTableToJsonString(config), "moonloader/config/AFK Informer.json")
			end
			if imgui.Checkbox(u8'Çâóêîå óâåäîìëåíèÿ', soundwindow) then
				config.settings.soundwindow = soundwindow[0]
				savejson(convertTableToJsonString(config), "moonloader/config/AFK Informer.json")
			end
			if imgui.CollapsingHeader(u8'Äîï.Íàñòðîéêè') then
				imgui.SetCursorPosX((imgui.GetWindowWidth() - 210) / 2)
				imgui.PushItemWidth(210)
				if imgui.InputTextWithHint(u8'##cmd', u8'Ââåäèòå êîìàíäó áåç "/"', cmdbuffer, sizeof(cmdbuffer), imgui.InputTextFlags.AutoSelectAll) then
					...
					if imgui.IsItemHovered() then
						...
						imgui.PushTextWrapPos(600)
							imgui.TextUnformatted(u8'×òîáû èçìåíèòü êîìàíäó àêòèâàöèè\nââåäèòå êîìàíäó áåç "/"')
						imgui.PopTextWrapPos()
						imgui.EndTooltip()
					end
				imgui.PopItemWidth()
				imgui.SetCursorPosX((imgui.GetWindowWidth() - 130) / 2)
				if imgui.Button(u8'Ñîõðàíèòü êîìàíäó', imgui.ImVec2(130, 0)) then
					config.settings.cmd = str(cmdbuffer)
				...
				if str(cmdbuffer) == nil or str(cmdbuffer) == '' or ffi.string(cmdbuffer) == ' ' or str(cmdbuffer):find('/.+') then
						changecmdtext = u8'Ïîëå ââîäà ïóñòîå èëè ñîäåðæèò ñèìâîë "/"\nÂâåäèòå êîìàíäó áåç "/" '
...
function main()
	...
	checksound()
	checklibs() -- ýòó óäàëèòü åñëè íå íóæíà ïðîâåðêà íà áèáëèîòåêè
	...
	sampSetClientCommandDescription(config.settings.cmd, string.format("Àêòèâàöèÿ/äåàêòèâàöèÿ îêíà %s, Ôàéë: %s", thisScript().name, thisScript().filename))
...
function secondThread()
	...
	if pushwindow[0] then
		ShowMessage('Âû ñòîèòå â àôê óæå '.. config.settings.time ..' ñåêóíä\nÂðåìÿ íà÷àëà àôê ' .. test ..' \nÒåêóùåå âðåìÿ '.. os.date('%X') ..'\n'..infoOK, 'AFK Informer')
	end
...
if lziplib then -- ñ ýòîé ñòðîêè è äî êîíöà
	function zipextract(script_name)
		...
		if doesFileExist(file_path) then
			...
			os.remove(file_path)
			print("Ðàñïàêîâêà ïðîøëà óñïåøíî, ðàñïàêîâàí àðõèâ: " .. script_name)
		else
			print("Ôàéëû íå íàéäåò, ïåðåçàïóñòèòå ñêðèïò.")
		end
	end
end
	...
	function MoveFiles(main_dir,dest_dir)
		for f in lfs.dir(main_dir) do
			...
			if doesFileExist(main_file) then
				...
				if doesFileExist(dest_file) then
					os.remove(main_file)
					print("Íåâîçìîæíî óäàëèòü ôàéë " .. dest_file)
				else
...
function checklibs()
	if not limgui or not llfs or not lziplib then
		lua_thread.create(function()
			print('Ïîäãðóçêà íåîáõîäèìûõ áèáëèîòåê..')
			if not lziplib then
			...
			print('Ïîäãðóçêà íåîáõîäèìûõ áèáëèîòåê îêîí÷åíà. Ïåðåçàãðóæàþñü..')
			wait(1000)
			reloadScripts()
		end)
		return false
	...
	if not doesFileExist(path) then
		downloadUrlToFile(link, path, function(id, status, p1, p2)
			if status == dlstatus.STATUSEX_ENDDOWNLOAD then
				print('{FFFFFF}Ôàéë {35ab2b}«'..name..'»{FFFFFF} çàãðóæåí!')
			end
		end)
	end
end
